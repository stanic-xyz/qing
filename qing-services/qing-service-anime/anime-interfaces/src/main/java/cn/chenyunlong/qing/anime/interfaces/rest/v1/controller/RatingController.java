...
public class RatingController {
    ...
    public ResponseEntity<Rating> addRating(
            ...
            @Parameter(description = "评分(1-10)") @RequestParam Integer score,
            @Parameter(description = "评论内容") @RequestParam(required = false) String comment,
            @Parameter(description = "是否匿名") @RequestParam(required = false, defaultValue = "false") Boolean isAnonymous,
            @Parameter(description = "是否公开") @RequestParam(required = false, defaultValue = "true") Boolean isPublic) {
        
        try {
            Rating rating = ratingService.addRating(userId, animeId, score, comment, isAnonymous, isPublic);
    ...
    public ResponseEntity<Void> likeRating(
        ...
        try {
            ratingService.likeRating(userId, animeId);
            return ResponseEntity.ok().build();
        } catch (IllegalStateException e) {
            return ResponseEntity.badRequest().build();
        }
    ...
    public ResponseEntity<Void> unlikeRating(
        ...
        try {
            ratingService.unlikeRating(userId, animeId);
            return ResponseEntity.ok().build();
        } catch (IllegalStateException e) {
            return ResponseEntity.badRequest().build();
        }
    ...
    public ResponseEntity<Void> setAnonymousStatus(
            @Parameter(description = "用户ID") @RequestParam Long userId,
            @Parameter(description = "动漫ID") @RequestParam Long animeId,
            @Parameter(description = "是否匿名") @RequestParam Boolean isAnonymous) {
