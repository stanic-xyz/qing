...
public class CmbParserTest extends BaseParserTest {
    ...
    public void testParsePdf() throws Exception {
        if (!resourceExists("cmb/cmb_test.pdf")) {
            System.out.println("⚠️ 找不到招商银行测试文件: mock/cmb/cmb_test.pdf");
            System.out.println("  请将匿名化后的招商银行交易流水 PDF 文件放入 src/test/resources/mock/cmb/cmb_test.pdf");
            return;
        }

        try (InputStream is = getResourceAsStream("cmb/cmb_test.pdf")) {
    ...
    public void testParseTxt() throws Exception {
        if (!resourceExists("cmb/cmb_test.pdf")) {
            System.out.println("⚠️ 找不到招商银行 TXT 测试文件: mock/cmb/cmb_test.pdf");
            System.out.println("  请将匿名化后的招商银行交易流水 TXT 文件放入 src/test/resources/mock/cmb/cmb_test.pdf");
            return;
        }
