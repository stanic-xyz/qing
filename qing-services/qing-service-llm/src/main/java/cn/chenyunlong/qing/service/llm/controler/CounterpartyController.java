...
public class CounterpartyController {
    ...
    @PutMapping("/{id}")
    public Result<Counterparty> update(@PathVariable Long id, @RequestBody Counterparty counterparty) {
        return counterpartyRepository.findById(id).map(existing -> {
            existing.setName(counterparty.getName());
            existing.setType(counterparty.getType());
            existing.setDefaultCategory(counterparty.getDefaultCategory());
