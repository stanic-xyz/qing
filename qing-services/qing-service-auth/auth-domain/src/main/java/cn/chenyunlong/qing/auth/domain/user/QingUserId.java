...
public class QingUserId extends EntityId<Long> implements Serializable {
     ...
     */
    private QingUserId(@NonNull Long value) {
        if (value <= 0) {
     ...
     * @throws IllegalArgumentException 当ID无效时
     */
    public static QingUserId of(Long value) {
        return new QingUserId(value);
    }
     ...
     */
    public static QingUserId of(String id) {
        if (id == null || id.trim().isEmpty()) {
            throw new IllegalArgumentException("分类ID字符串不能为空");
        }
        try {
            return new QingUserId(Long.parseLong(id.trim()));
