...
export default function CounterPartySelector({isOpen, onClose, onSelect}: CounterPartySelectorProps) {
    ...
    const fetchTransactions = async () => {
        ...
        try {
            ...
            if (res.data.code === 200) {
                setCounterparties(res.data.data.content || []);
            }
        } catch (e) {
            console.error(e);
        } finally {
    ...
    const handleConfirm = () => {
        if (selectedId) {
            const record = counterparties.find(t => t.id === selectedId);
            if (record) {
                onSelect(record);
            }
        }
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center" aria-labelledby="modal-title" role="dialog"
             aria-modal="true">
            <div className="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity" aria-hidden="true"
                 onClick={onClose}></div>
            <div
                className="relative bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all w-full max-w-4xl max-h-[90vh] flex flex-col">
                <div className="bg-white px-4 pt-5 pb-4 sm:p-6 flex-1 overflow-hidden flex flex-col">
                    ...
                    <div className="flex-1 overflow-y-auto border rounded-md">
                        {loading ? (
                            ...
                            <div className="p-4 text-center text-gray-500">未找到匹配的交易对手</div>
                        ) : (
                            <table className="min-w-full divide-y divide-gray-200 relative">
                                ...
                                <tr>
                                    ...
                                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider bg-gray-50">类型</th>
                                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider bg-gray-50">商户名</th>
                                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider bg-gray-50">支付方式</th>
