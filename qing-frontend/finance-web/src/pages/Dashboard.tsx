  ...
  return (
    <div className="flex flex-col h-full space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* 趋势图 */}
        <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-100 lg:col-span-2 flex flex-col min-h-[400px]">
            ...
            {stats.trends && stats.trends.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={stats.trends}>
                  ...
                  <Line type="monotone" name="收入" dataKey="income" stroke="#16a34a" strokeWidth={3} dot={false} activeDot={{r: 6}} />
                  <Line type="monotone" name="支出" dataKey="expense" stroke="#dc2626" strokeWidth={3} dot={false} activeDot={{r: 6}} />
                </LineChart>
              </ResponsiveContainer>
            ) : (
        ...
        {stats.accounts && stats.accounts.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {stats.accounts.map((acc: any) => (
              <div key={acc.id} className="p-4 border border-gray-100 rounded-lg hover:shadow-md transition-shadow bg-gray-50 flex items-center justify-between">
