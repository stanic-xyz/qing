...
export default function TransactionTrace() {
    ...
    return (
        <div className="flex flex-col h-full gap-4">
            <div>
                <h1 className="text-2xl font-bold mb-2">交易轨迹</h1>
            ...
            <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-100">
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div>
                        ...
                        <select name="compareStatus" value={filters.compareStatus} onChange={handleFilterChange}
                                className="w-full border-gray-300 rounded-md shadow-sm p-2 text-sm border focus:ring-blue-500 focus:border-blue-500">
                ...
                <div className="mt-4 flex justify-end space-x-3">
                    ...
                    <button onClick={handleSearch}
                            className="flex items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700 focus:outline-none">
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-100 flex-1 overflow-auto">
                <table className="min-w-full divide-y divide-gray-200">
                    ...
                    <tr>
                        <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">基础信息
                            (日期/金额)
                        </th>
                        {CHANNELS.map(ch => (
                            <th key={ch}
                                className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">{getEnumText('channel', ch)}</th>
                        ))}
                    </tr>
                    </thead>
                            ...
                            <React.Fragment key={group.groupId}>
                                ...
                                >
                                    <td className="px-4 py-4 whitespace-nowrap border-b border-gray-100">
                                        ...
                                        <div
                                            className={`text-sm font-bold mt-1 ${group.mainType === 'INCOME' ? 'text-green-600' : 'text-red-600'}`}>
                                            {group.mainAmount}
                                        </div>
                                    </td>
                                    {CHANNELS.map(ch => {
                                        ...
                                        return (
                                            <td key={ch} className="px-4 py-4 border-b border-gray-100">
                                                {hasRecord ? (
                                                    <span
                                                        className="inline-block px-2 py-1 bg-green-100 text-green-800 text-xs rounded-full">有记录</span>
                                                ) : (
                                ...
                                {isExpanded && (
                                    <tr>
                                        <td colSpan={CHANNELS.length + 1}
                                            className="p-0 border-b-2 border-blue-200 bg-gray-50">
                                            <div className="p-4">
                                                <h4 className="text-sm font-bold text-gray-700 mb-3 pl-2 border-l-4 border-blue-500">详细渠道数据比对</h4>
                                                <table
                                                    className="min-w-full bg-white border border-gray-200 rounded-md overflow-hidden shadow-sm">
                                                    ...
                                                    {Object.entries(group.channelRecords).map(([ch, record]: [string, any]) => (
                                                        <tr key={ch} className="hover:bg-gray-50">
                                                            ...
                                                            <td className="px-4 py-3 text-sm text-gray-800 max-w-xs truncate"
                                                                title={record.counterparty?.name}>{record.counterparty?.name || '-'}</td>
                                                            <td className="px-4 py-3 text-sm text-gray-600">
                                                                <div className="max-w-md truncate"
                                                                     title={record.merchant}>说明: {record.merchant || '-'}</div>
                                                                {record.remark && <div
                                                                    className="max-w-md truncate text-gray-400 mt-1"
                                                                    title={record.remark}>备注: {record.remark}</div>}
                                                            </td>
                                                        </tr>
                                                    ))}
                                                    </tbody>
