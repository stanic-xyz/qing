            ...
            {Object.keys(groupedMatchers).map(type => {
              ...
              return (
                <React.Fragment key={type}>
                  {/* 分组头 */}
                  <tr className="bg-gray-100 cursor-pointer hover:bg-gray-200" onClick={() => toggleGroup(type)}>
                    <td colSpan={6} className="px-6 py-2 text-sm font-semibold text-gray-700 flex items-center">
                      {isExpanded ? <ChevronDown size={16} className="mr-2" /> : <ChevronRight size={16} className="mr-2" />}
                      {type} <span className="ml-2 text-xs font-normal text-gray-500">({groupRules.length} 个规则)</span>
                    </td>
                  </tr>
            ...
            <div className="grid grid-cols-3 gap-6">
              ...
              <div className="col-span-2 space-y-6">
                ...
                <RuleBuilder
                  ...
                  onChangeCondition={(node) => setEditingMatcher({ ...editingMatcher, conditionNode: node })}
                  actionNode={editingMatcher.actionNode}
                  onChangeAction={(actions) => setEditingMatcher({ ...editingMatcher, actionNode: actions })}
                />
              </div>
              ...
              <div className="col-span-1 space-y-6 border-l pl-6">
                <div className="space-y-4">
                  ...
                  <div className="text-sm">
                    <span className="font-medium text-gray-700 block mb-1">所属目标 (多态绑定)</span>
                    {editingMatcher.targetType && editingMatcher.targetId ? (
                      <div className="bg-gray-50 border border-gray-200 rounded px-3 py-2 flex items-center justify-between">
                      ...
                      <div className="bg-gray-50 border border-gray-200 rounded px-3 py-2 text-gray-500 italic">
                        全局通用规则 (未绑定特定实体)
                      </div>
                    )}
                  </div>
                  ...
                  <div className="flex flex-col gap-2 pt-2">
                    <label className="flex items-center cursor-pointer">
                      <input type="checkbox" checked={editingMatcher.stopOnMatch} onChange={e => setEditingMatcher({...editingMatcher, stopOnMatch: e.target.checked})} className="text-blue-600 rounded mr-2" />
                      <span className="text-sm text-gray-700">命中后阻断 (阻止低优规则)</span>
                    </label>
                    <label className="flex items-center cursor-pointer">
                      <input type="checkbox" checked={editingMatcher.isActive} onChange={e => setEditingMatcher({...editingMatcher, isActive: e.target.checked})} className="text-blue-600 rounded mr-2" />
                ...
                <div className="pt-4 border-t border-gray-200">
                  ...
                  <div className="bg-gray-50 p-3 rounded border border-gray-200 mb-3">
                    {testTransaction ? (
                      <div className="text-sm space-y-1">
                        <div className="font-medium truncate" title={testTransaction.merchant || testTransaction.counterparty}>{testTransaction.merchant || testTransaction.counterparty || '无对手方'}</div>
                        <div className="text-gray-500 flex justify-between">
                          <span>{testTransaction.transactionTime?.split(' ')[0]}</span>
                          <span className="font-medium text-gray-900">{testTransaction.amount}</span>
                        </div>
                        <button onClick={() => setShowTransactionSelector(true)} className="text-blue-600 text-xs mt-2 hover:underline">重新选择</button>
                      </div>
                    ) : (
                      <div className="text-center py-4">
                        <button onClick={() => setShowTransactionSelector(true)} className="text-blue-600 text-sm flex items-center justify-center w-full hover:underline">
                  ...
                  {testResult && (
                    <div className={`mt-3 p-3 rounded text-sm ${testResult.matched ? 'bg-green-50 border border-green-200' : 'bg-red-50 border border-red-200'}`}>
                      {testResult.matched ? (
                         <div>
                           <p className="font-bold text-green-800 mb-2">✅ 匹配成功！</p>
                           <div className="space-y-1 text-xs text-gray-700">
                             <p><strong>修改前:</strong> {testResult.originalRecord.type} | {testResult.originalRecord.category || '无分类'}</p>
                             <p><strong>修改后:</strong> {testResult.modifiedRecord.type} | {testResult.modifiedRecord.category || '无分类'}</p>
                             {testResult.modifiedRecord.counterparty !== testResult.originalRecord.counterparty && (
                               <p><strong>新对手:</strong> {testResult.modifiedRecord.counterparty}</p>
                             )}
                           </div>
                         </div>
                       ) : (
