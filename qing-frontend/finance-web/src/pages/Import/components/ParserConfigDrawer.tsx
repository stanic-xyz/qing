...
export default function ParserConfigDrawer({isOpen, onClose}: ParserConfigDrawerProps) {
    ...
    const [ruleTestStatus, setRuleTestStatus] = useState<Record<number, {
        loading: boolean;
        result?: any;
        error?: string
    }>>({});
    ...
    const handlePublishStatusChange = async (config: ParserConfig, isPublished: boolean) => {
        if (isPublished) {
            await configApi.publish(config.id);
        } else {
            await configApi.unpublish(config.id);
        }
        await fetchConfigs();
    };
                        ...
                        {editingId && (
                            <div className="w-1/2 border-r bg-gray-50 flex flex-col overflow-hidden">
                                <div className="p-4 border-b bg-white">
                                    ...
                                    <input type="file" accept={formData.fileType === 'CSV' ? '.csv' : '.xlsx,.xls'}
                                           onChange={handleTestFileChange}
                                           className="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100"/>
                                    <button onClick={handleTest} disabled={!testFile || isTesting}
                                            className="mt-2 px-4 py-2 bg-green-600 text-white rounded shadow-sm text-sm disabled:opacity-50">
                                        {isTesting ? '测试中...' : '运行测试'}
                                    </button>
                                </div>
                          ...
                          {JSON.stringify(testResult.metadata, null, 2)}
                                            ...
                                            <div className="bg-white p-3 rounded border shadow-sm">
                                                <h4 className="font-medium text-gray-700 border-b pb-2 mb-2">解析记录
                                                    ({testResult.records?.length || 0}条)</h4>
                                                        ...
                                                        {testResult.records?.slice(0, 20).map((r: any, i: number) => (
                                                            <tr key={i}>
                                                                ...
                                                                <td className="px-2 py-1 truncate max-w-[150px]"
                                                                    title={r.originalData}>{r.originalData}</td>
                                                            </tr>
                                                        ))}
                                                        </tbody>
                                                    </table>
                                                    {testResult.records?.length > 20 && <div
                                                        className="text-center text-gray-500 py-2">只显示前20条</div>}
                                                </div>
                                            </div>
                                        </div>
                                    )}
                                ...
                                <div className="space-y-6">
                                    ...
                                    <div className="bg-gray-50 p-4 rounded-lg space-y-4">
                                        ...
                                        <div className="grid grid-cols-2 gap-4">
                                            <div>
                                                <label
                                                    className="block text-sm font-medium text-gray-700">解析器名称</label>
                                            ...
                                            <div>
                                                ...
                                                <select
                                                    ...
                                                    onChange={e => setFormData({
                                                        ...formData,
                                                        channel: {code: e.target.value} as any
                                                    })}
                                                    className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2"
                                                >

                                                    {CHANNEL_OPTIONS.map(c => <option key={c}
                                                                                      value={c}>{c}</option>)}

                                                </select>
                                                {/*<datalist id="channel-options">*/}
                                                {/*    {CHANNEL_OPTIONS.map(c => <option key={c} value={c} >我们发{c}</option>)}*/}
                                                {/*</datalist>*/}
                                            </div>
                                            <div>
                                                <label
                                                    className="block text-sm font-medium text-gray-700">文件类型</label>
                                                <select value={formData.fileType || ''} onChange={e => setFormData({
                                                    ...formData,
                                            ...
                                            <div>
                                                ...
                                                <select value={formData.encoding || ''} onChange={e => setFormData({
                                                    ...formData,
                                            ...
                                            <div>
                                                <label className="block text-sm font-medium text-gray-700">跳过行数
                                                    (数据起始行)</label>
                                                <input type="number" value={formData.skipRows || 0}
                                                       onChange={e => setFormData({
                                                           ...formData,
                                            ...
                                            <div>
                                                <label
                                                    className="block text-sm font-medium text-gray-700">来源类型</label>
                                                <select value={formData.sourceType || ''} onChange={e => setFormData({
                                                    ...formData,
                                                    sourceType: e.target.value
                                                })}
                                                        className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2">
                                                    <option value="钱迹">钱迹</option>
                                    ...
                                    <div className="bg-gray-50 p-4 rounded-lg">
                                        <div className="flex justify-between items-center mb-4">
                                            ...
                                            <button onClick={() => setMetadataRules([...metadataRules, {
                                                targetField: '',
                                                rowIndex: 0,
                                                colIndex: 0,
                                                regex: ''
                                        ...
                                        {metadataRules.map((rule, idx) => (
                                            <div key={idx} className="flex items-center space-x-2 mb-2">
                                                <select value={rule.targetField} onChange={e => {
                                                    const n = [...metadataRules];
                                                    n[idx].targetField = e.target.value;
                                                    setMetadataRules(n);
                                                }} className="border rounded p-1.5 text-sm w-1/4">
                                                ...
                                                <input type="number" placeholder="行号" value={rule.rowIndex}
                                                       onChange={e => {
                                                           const n = [...metadataRules];
                                                           n[idx].rowIndex = parseInt(e.target.value);
                                                           setMetadataRules(n);
                                                       }} className="border rounded p-1.5 text-sm w-1/6"
                                                       title="行号(从0开始)"/>
                                                <input type="number" placeholder="列号" value={rule.colIndex}
                                                       onChange={e => {
                                                           const n = [...metadataRules];
                                                           n[idx].colIndex = parseInt(e.target.value);
                                                           setMetadataRules(n);
                                                       }} className="border rounded p-1.5 text-sm w-1/6"
                                                       title="列号(从0开始)"/>
                                                <input type="text" placeholder="正则提取(选填)" value={rule.regex || ''}
                                                       onChange={e => {
                                                           const n = [...metadataRules];
                                                           n[idx].regex = e.target.value;
                                                           setMetadataRules(n);
                                                       }} className="border rounded p-1.5 text-sm flex-1"
                                                       title="如: \[(\d+)\]"/>
                                                <button
                                                    onClick={() => setMetadataRules(metadataRules.filter((_, i) => i !== idx))}
                                                    className="text-red-500 hover:text-red-700"><Trash size={16}/>
                                                </button>
                                            </div>
                                        ))}
                                        {metadataRules.length === 0 &&
                                            <div className="text-sm text-gray-500">暂无规则，将尝试自动推算。</div>}
                                    </div>
                                    ...
                                    <div className="bg-gray-50 p-4 rounded-lg">
                                        <div className="flex justify-between items-center mb-4">
                                            ...
                                            <button onClick={() => setFieldRules([...fieldRules, {
                                                targetField: '',
                                                sourceIndex: 0,
                                                dateFormat: '',
                                                cleanRules: [],
                                        ...
                                        {fieldRules.map((rule, idx) => (
                                            <div key={idx}
                                                 className="flex flex-wrap items-center gap-2 mb-3 p-2 border border-dashed rounded bg-white">
                                                <div className="flex w-full space-x-2 items-center">
                                                    <select value={rule.targetField} onChange={e => {
                                                        ...
                                                        <option value="transactionTime">交易时间 (transactionTime)
                                                        </option>
                                                        ...
                                                        <option value="extData.counterpartyAccount">扩展:对方账号
                                                            (extData.counterpartyAccount)
                                                        </option>
                                                        <option value="extData.counterpartyName">扩展:对方名称
                                                            (extData.counterpartyName)
                                                        </option>
                                                        <option value="extData.goodsDescription">扩展:商品说明
                                                            (extData.goodsDescription)
                                                        </option>
                                                        <option value="extData.tradeOrderNo">扩展:交易订单号
                                                            (extData.tradeOrderNo)
                                                        </option>
                                                        <option value="extData.merchantOrderNo">扩展:商家订单号
                                                            (extData.merchantOrderNo)
                                                        </option>
                                                        <option value="extData.paymentMethod">扩展:收付款方式
                                                            (extData.paymentMethod)
                                                        </option>
                                                        <option value="extData.cardLast4">扩展:卡号后四位
                                                            (extData.cardLast4)
                                                        </option>
                                                        <option value="extData.billingAmount">扩展:结算金额
                                                            (extData.billingAmount)
                                                        </option>
                                                        <option value="extData.billingCurrency">扩展:结算币种
                                                            (extData.billingCurrency)
                                                        </option>
                                                    </select>
                                                    <input type="number" placeholder="列号(0开始)"
                                                           value={rule.sourceIndex} onChange={e => {
                                                        const n = [...fieldRules];
                                                        n[idx].sourceIndex = parseInt(e.target.value);
                                                        setFieldRules(n);
                                                    }} className="border rounded p-1.5 text-sm w-24"
                                                           title="CSV列号(从0开始)"/>
                                                    <input type="text" placeholder="默认值(选填)"
                                                           value={rule.defaultValue || ''} onChange={e => {
                                                        const n = [...fieldRules];
                                                        n[idx].defaultValue = e.target.value;
                                                        setFieldRules(n);
                                                    }} className="border rounded p-1.5 text-sm w-32"/>
                                                    <button
                                                        onClick={() => setFieldRules(fieldRules.filter((_, i) => i !== idx))}
                                                        className="text-red-500 hover:text-red-700 ml-auto"><Trash
                                                        size={16}/></button>
                                                </div>
                                                ...
                                                <div className="flex w-full space-x-2 items-center mt-1">
                                                    {rule.targetField === 'transactionTime' && (
                                                        <input type="text"
                                                               ...
                                                               value={rule.dateFormat || ''} onChange={e => {
                                                            const n = [...fieldRules];
                                                            n[idx].dateFormat = e.target.value;
                                                            setFieldRules(n);
                                                        }} className="border rounded p-1.5 text-sm flex-1"/>
                                                    )}
                                                    <div className="flex items-center space-x-3 text-sm text-gray-600">
                                                        ...
                                                        <label className="flex items-center"><input type="checkbox"
                                                                                                    className="mr-1"
                                                                                                    checked={rule.cleanRules?.includes('TRIM')}
                                                                                                    onChange={e => {
                                                                                                        const n = [...fieldRules];
                                                                                                        if (e.target.checked) {
                                                                                                            n[idx].cleanRules = [...(n[idx].cleanRules || []), 'TRIM']
                                                                                                        } else {
                                                                                                            n[idx].cleanRules = (n[idx].cleanRules || []).filter(x => x !== 'TRIM')
                                                                                                        }
                                                                                                        ;setFieldRules(n);
                                                                                                    }}/> 去空白</label>
                                                        <label className="flex items-center"><input type="checkbox"
                                                                                                    className="mr-1"
                                                                                                    checked={rule.cleanRules?.includes('REMOVE_RMB')}
                                                                                                    onChange={e => {
                                                                                                        const n = [...fieldRules];
                                                                                                        if (e.target.checked) {
                                                                                                            n[idx].cleanRules = [...(n[idx].cleanRules || []), 'REMOVE_RMB']
                                                                                                        } else {
                                                                                                            n[idx].cleanRules = (n[idx].cleanRules || []).filter(x => x !== 'REMOVE_RMB')
                                                                                                        }
                                                                                                        ;setFieldRules(n);
                                                                                                    }}/> 去¥</label>
                                                        <label className="flex items-center"><input type="checkbox"
                                                                                                    className="mr-1"
                                                                                                    checked={rule.cleanRules?.includes('REMOVE_COMMAS')}
                                                                                                    onChange={e => {
                                                                                                        const n = [...fieldRules];
                                                                                                        if (e.target.checked) {
                                                                                                            n[idx].cleanRules = [...(n[idx].cleanRules || []), 'REMOVE_COMMAS']
                                                                                                        } else {
                                                                                                            n[idx].cleanRules = (n[idx].cleanRules || []).filter(x => x !== 'REMOVE_COMMAS')
                                                                                                        }
                                                                                                        ;setFieldRules(n);
                                                                                                    }}/> 去逗号</label>
                                                        <label className="flex items-center"><input type="checkbox"
                                                                                                    className="mr-1"
                                                                                                    checked={rule.cleanRules?.includes('REMOVE_TABS')}
                                                                                                    onChange={e => {
                                                                                                        const n = [...fieldRules];
                                                                                                        if (e.target.checked) {
                                                                                                            n[idx].cleanRules = [...(n[idx].cleanRules || []), 'REMOVE_TABS']
                                                                                                        } else {
                                                                                                            n[idx].cleanRules = (n[idx].cleanRules || []).filter(x => x !== 'REMOVE_TABS')
                                                                                                        }
                                                                                                        ;setFieldRules(n);
                                                                                                    }}/> 去隐藏制表符</label>
                                                    </div>
                          ...
                          />
                                                        <div className="mt-2 flex gap-2 items-center">
                                                            <input
                                                                ...
                                                                onChange={e => setRuleTestValue(prev => ({
                                                                    ...prev,
                                                                    [idx]: e.target.value
                                                                }))}
                                                        ...
                                                        {ruleTestStatus[idx]?.error && (
                                                            <div
                                                                className="mt-1 text-xs text-red-600">{ruleTestStatus[idx]?.error}</div>
                                                        )}
                                                        {ruleTestStatus[idx]?.result !== undefined && !ruleTestStatus[idx]?.error && (
                                                            <div
                                                                className="mt-1 text-xs text-green-700">结果: {String(ruleTestStatus[idx]?.result)}</div>
                                                        )}
                                                    </div>
                                                )}
                                            </div>
                                        ...
                                        {fieldRules.length === 0 &&
                                            <div className="text-sm text-gray-500">暂无映射规则。</div>}
                                    </div>
                                        ...
                                        <div className="flex items-center gap-3">
                                            <label className="flex items-center gap-2 text-sm text-gray-700">
                                                <input
                                                    ...
                                                    onChange={e => setFormData({
                                                        ...formData,
                                                        postScriptEnabled: e.target.checked
                                                    })}
                                                />
                                                启用后置脚本（字段映射完成后执行）
                                            </label>
                                            <select
                                                ...
                                                onChange={e => setFormData({
                                                    ...formData,
                                                    postScriptLanguage: e.target.value
                                                })}
                                            >
                                                <option value="groovy">Groovy</option>
                                            </select>
...
`
                                        ...
                                        <p className="text-xs text-gray-500">
                                            用途：把“金额→收支类型”等跨字段逻辑放到这里；字段映射脚本建议仅返回标量值（如金额
                                            BigDecimal）。
                                        </p>
                                    </div>
                                    ...
                                    <div className="flex justify-end space-x-3 pt-4 border-t">
                                        <button onClick={() => setEditingId(null)}
                                                className="px-4 py-2 border rounded-md text-gray-700 hover:bg-gray-50">取消
                                        </button>
                                        <button onClick={() => handleSave('DRAFT')}
                                                className="px-4 py-2 bg-yellow-500 hover:bg-yellow-600 text-white rounded-md flex items-center shadow-sm">保存草稿
                                        </button>
                                        <button onClick={() => handleSave('PUBLISHED')}
                                                className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-md flex items-center shadow-sm">
                                            <Save size={16} className="mr-1"/> 发布
                                        </button>
                                    </div>
                                </div>
                            ) : (
                                <div>
                                    ...
                                    <div className="space-y-3">
                                        {configs.map(config => {
                                            ...
                                            // 判断是否可以编辑：非内置且草稿状态
                                            const canEdit = !isBuiltIn && isDraft;
                                            // 判断是否可以删除：非内置且草稿状态
                                            const canDelete = !isBuiltIn && isDraft;
                                            // 判断是否可以发布：非内置且草稿状态
                                            const canPublish = !isBuiltIn && isDraft;
                                            // 判断是否可以取消发布：非内置且已发布状态
                                            const canUnpublish = !isBuiltIn && isPublished;
                                                ...
                                                <div key={config.id}
                                                    ...
                                                    <div>
                                                        <h4 className="font-medium text-gray-900">
                                                            ...
                                                            {isBuiltIn && (
                                                                <span
                                                                    className="text-xs bg-gray-200 text-gray-600 px-2 py-0.5 rounded ml-2">内置</span>
                                                            )}
                                                            {isDraft && !isBuiltIn && (
                                                                <span
                                                                    className="text-xs bg-yellow-100 text-yellow-800 px-2 py-0.5 rounded ml-2">草稿</span>
                                                            )}
                                                            {isPublished && !isBuiltIn && (
                                                                <span
                                                                    className="text-xs bg-green-100 text-green-800 px-2 py-0.5 rounded ml-2">已发布</span>
                                                            )}
                                                        </h4>
                                                        <p className="text-sm text-gray-500 mt-1">
                                                            渠道: {config.channel?.name || config.channel?.code || '未知'} |
                                                            类型: {config.fileType} | 编码: {config.encoding}
                                                        </p>
                                                    </div>
                                                    <div className="flex space-x-2">
                                                        {canPublish && (
                                                            <button
                                                                onClick={() => handlePublishStatusChange(config, true)}
                                                        ...
                                                        {canUnpublish && (
                                                            <button
                                                                onClick={() => handlePublishStatusChange(config, false)}
                                                                className="text-blue-600 hover:text-blue-800 text-sm font-medium">
                                                                取消发布
                                                            </button>
                                                        )}
                                                        <button
                                                            onClick={() => canEdit && handleEdit(config)}
                                                            disabled={!canEdit}
                                                            className={`text-sm font-medium ${canEdit ? 'text-blue-600 hover:text-blue-800' : 'text-gray-400 cursor-not-allowed'}`}
                                                        >
                                                            编辑
                                                        </button>
                                                        <button
                                                            onClick={() => canDelete && handleDelete(config.id)}
                                                            disabled={!canDelete}
