...
export default function Categories() {
  ...
  const renderTypeTag = (type: string) => {
    switch(type) {
      ...
      case 'INCOME': return <span className="px-2 py-1 bg-green-100 text-green-700 text-xs rounded-full">收入</span>;
      case 'TRANSFER': return <span className="px-2 py-1 bg-blue-100 text-blue-700 text-xs rounded-full">转账</span>;
      default: return null;
    }
  };
  ...
  const renderTreeNodes = (nodes: Category[]) => {
    return nodes.map(node => (
      <div key={node.id} className="border-b border-gray-100 last:border-0">
        <div className={`flex items-center justify-between p-3 hover:bg-gray-50 ${node.level > 0 ? 'pl-10 bg-gray-50/50' : ''}`}>
          <div className="flex items-center space-x-3">
            {node.children && node.children.length > 0 ? (
              <button onClick={() => toggleExpand(node.id)} className="text-gray-500 hover:text-gray-700 focus:outline-none">
          ...
          <div className="flex space-x-2">
            ...
            <button onClick={() => handleEdit(node)} className="p-1 text-gray-600 hover:bg-gray-200 rounded" title="编辑">
              <Edit2 size={16} />
            </button>
            <button onClick={() => handleDelete(node.id)} className="p-1 text-red-600 hover:bg-red-50 rounded" title="删除">
              <Trash2 size={16} />
            </button>
          </div>
        </div>
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        ...
        <button onClick={() => handleAdd()} className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700">
          <Plus size={16} />
          新增主分类
        </button>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
        <div className="flex items-center justify-between p-4 bg-gray-50 border-b border-gray-200 text-sm font-medium text-gray-600">
          <div>分类名称</div>
          <div>操作</div>
        </div>
        {loading ? (
