...
public class UserService {
    ...
    public UserInfo getUserInfo(String authCode) throws Exception {
        try {
            DingTalkClient client =
                new DefaultDingTalkClient("https://oapi.dingtalk.com/topapi/v2/user/getuserinfo");
            OapiV2UserGetuserinfoRequest request = new OapiV2UserGetuserinfoRequest();
            request.setCode(authCode);

            OapiV2UserGetuserinfoResponse rsp =
                client.execute(request, accessTokenService.getAccessToken());
            if (!rsp.isSuccess()) {
                log.error(
                    "UserService_getUserInfo getUserInfo failed, errorCode={}, errorMessage={}.",
                    rsp.getErrcode(), rsp.getErrmsg());
                return null;
            }

            OapiV2UserGetuserinfoResponse.UserGetByCodeResponse userGetByCodeResponse =
                rsp.getResult();
            ...
            log.info("UserService_getUserInfo get success, userid={}, avatar={}.",
                userInfo.getUserId(), userInfo.getAvatar());
            return userInfo;
        } catch (Exception e) {
    ...
    private String getAvatar(String userid) throws ApiException {
        DingTalkClient client =
            new DefaultDingTalkClient("https://oapi.dingtalk.com/topapi/v2/user/get");
        OapiV2UserGetRequest req = new OapiV2UserGetRequest();
        req.setUserid(userid);
        req.setLanguage("zh_CN");

        OapiV2UserGetResponse userGetResponse =
            client.execute(req, accessTokenService.getAccessToken());
